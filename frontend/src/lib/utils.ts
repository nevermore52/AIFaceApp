...
const ERROR_MAP: Array<[RegExp | string, string]> = [
  ...
  // KieAPI model-specific errors
  [/upper body not detected/i,      'Нейросеть не смогла найти верхнюю часть тела на фото. Используйте портрет или фото по пояс, где хорошо видны голова, плечи и туловище.'],
  [/face not detected|no face/i,    'Нейросеть не смогла найти лицо на фото. Используйте чёткое фото, где лицо хорошо видно и не перекрыто.'],
  [/multiple faces|more than one face/i, 'На фото обнаружено несколько лиц. Для генерации используйте фото с одним человеком.'],
  [/public figure/i,                'На фото обнаружена известная личность. Генерация с публичными персонами недоступна.'],
